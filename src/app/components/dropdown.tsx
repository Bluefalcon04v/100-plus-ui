  ...
  return (
    <div className="top-8 z-20 fixed text-black">
      <button
        ...
        className={`bg-white px-2 hover:bg-neutral-300 py-2 border-white rounded-md w-fit font-semibold  text-sm active:scale-95 transition-all cursor-pointer border-2 dropdown-starting }`}
      >
        <div className="flex place-content-center place-items-center gap-2 transition-all duration-200">
          ...
          <DropdownIcon
            className={`transition-all duration-300 ${
              !dropdownToggle ? "rotate-0 " : "rotate-180"
            }`}
          />
        </div>
      </button>
      {dropdownToggle && (
        <div className="relative flex flex-col gap-2 bg-white/30 my-2 mt-1 px-2 py-3 rounded-md w-full max-h-80 font-medium text-sm dropdown-starting">
          <input
            ...
            className="top-0 fixed bg-white mt-2 px-2 py-2 border-transparent rounded-md outline-amber-400 focus:outline-[3px] w-11/12"
          />
